...
For Thingino PTZ cameras, Settings displays motor health and the current motor position. Double-click-center ("home") always reads the camera's persistent `motors.pos_0` value from `/etc/thingino.json`, the position used after camera reboots. A single-click center instead drives to the camera's own explicit `center_x`/`center_y` coordinates, if one has been captured — click "Set center to current position" in Settings after aiming the camera where you want it (via the D-pad or manually) to capture and save that position; ONVIF cameras capture their PTZ absolute pan/tilt position the same way. Cameras that haven't captured a center yet fall back to `pos_0` (Thingino) or pan/tilt `(0, 0)` (ONVIF), matching the prior behavior. Center and double-click-center operations use a longer timeout because an absolute move can take substantially longer than a directional step.
...
| POST | `/api/cameras/{id}/ptz` | Body `{"command": "left\|right\|up\|down\|up-left\|up-right\|down-left\|down-right\|center\|home", "coarse": false}`. `center` drives to the camera's saved `center_x`/`center_y`, falling back to the camera's own home position if none is saved. |
| POST | `/api/cameras/{id}/ptz/set-center` | Detects the camera's current pan/tilt position and saves it as `center_x`/`center_y` for that camera. |
