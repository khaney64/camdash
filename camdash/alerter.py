...
class AlertEngine:
    ...
    def evaluate(self, event: dict[str, Any], thumb: Path | None, cooldown_seconds: int) -> dict[str, Any]:
    ...
    def _email(self, event: dict[str, Any], rule: str, thumb: Path | None) -> None:
        detections = ", ".join(d.get("label", "") for d in (event.get("analysis") or {}).get("detections", []))
        self._send(
            f"CAM Dashboard: {detections or rule} at {event['camera_name']}",
    ...
    def _send(subject: str, body: str, thumb: Path | None) -> None:
        ...
        if thumb and thumb.exists():
            message.add_attachment(thumb.read_bytes(), maintype="image", subtype="jpeg", filename="capture.jpg")
