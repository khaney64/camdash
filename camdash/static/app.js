...
function renderCameraSettings(){
  $('camera-settings').innerHTML=S.cameras.map((c,i)=>`<form class="camera-form panel" data-index="${i}"><div class="camera-form-head"><h3>${esc(c.name||'New camera')}</h3><button type="button" class="button danger" data-remove-camera="${i}">Remove</button></div><div class="field-grid">
    ...
    ${field('center_x','PTZ center X',c.center_x,'number')}${field('center_y','PTZ center Y',c.center_y,'number')}
    ...
    </div><div class="camera-actions"><button type="button" class="button" data-probe-camera="${esc(c.id)}">Probe ONVIF</button><button type="button" class="button" data-sd-camera="${esc(c.id)}">Check SD</button>${c.adapter==='thingino'&&c.ptz?`<button type="button" class="button" data-motor-status="${esc(c.id)}">Check motors</button><span class="message motor-health" data-motor-result="${esc(c.id)}">Checking motors…</span>`:''}${c.ptz?`<button type="button" class="button" data-set-center-camera="${esc(c.id)}">Set center to current position</button><button type="button" class="button" data-recenter-camera="${esc(c.id)}">Recenter</button>`:''}<span class="message" data-camera-result="${esc(c.id)}"></span></div></form>`).join('');
...
async function setCameraCenter(id,form){
  const out=document.querySelector(`[data-camera-result="${CSS.escape(id)}"]`);if(out)out.textContent='Detecting position…';
  try{
    const data=await api(`/api/cameras/${id}/ptz/set-center`,{method:'POST'});
    if(form){const cx=form.elements.namedItem('center_x'),cy=form.elements.namedItem('center_y');if(cx)cx.value=data.center_x;if(cy)cy.value=data.center_y;}
    const idx=form?Number(form.dataset.index):-1;if(idx>=0&&S.cameras[idx])Object.assign(S.cameras[idx],{center_x:data.center_x,center_y:data.center_y});
    if(out)out.textContent=`Center set to (${data.center_x}, ${data.center_y})`;
    toast('Center position saved');
  }catch(e){if(out)out.textContent=e.message;toast(e.message,true);}
}
async function recenterCamera(id){
  const out=document.querySelector(`[data-camera-result="${CSS.escape(id)}"]`);if(out)out.textContent='Recentering…';
  try{await api(`/api/cameras/${id}/ptz`,{method:'POST',body:JSON.stringify({command:'center',coarse:false})});if(out)out.textContent='Recenter command sent';toast('Recentering camera');}
  catch(e){if(out)out.textContent=e.message;toast(e.message,true);}
}
...
function bind() {
  ...
  $('camera-settings').onclick=e=>{const remove=e.target.closest('[data-remove-camera]');if(remove){S.cameras.splice(Number(remove.dataset.removeCamera),1);renderCameraSettings();return}const probe=e.target.closest('[data-probe-camera]');if(probe)probeCamera(probe.dataset.probeCamera);const sd=e.target.closest('[data-sd-camera]');if(sd)checkSd(sd.dataset.sdCamera);const motorStatus=e.target.closest('[data-motor-status]');if(motorStatus)checkMotors(motorStatus.dataset.motorStatus);const setCenter=e.target.closest('[data-set-center-camera]');if(setCenter)setCameraCenter(setCenter.dataset.setCenterCamera,setCenter.closest('.camera-form'));const recenter=e.target.closest('[data-recenter-camera]');if(recenter)recenterCamera(recenter.dataset.recenterCamera);};
